...
pub fn verify_niceness(candidate_string: &str) -> bool {
    ...
    let lookahead_pair = candidate_string.chars().zip(candidate_string.chars().skip(1));

    let matching_pair = lookahead_pair.clone().any(|(a, b)| a == b);
    
    let illegal_pairs = [('a', 'b'), ('c', 'd'), ('p', 'q'), ('x', 'y')];
    let no_illegal_pairs = lookahead_pair.clone().all(|(a, b)| !illegal_pairs.contains(&(a, b)));
    vowel_count_check && matching_pair && no_illegal_pairs
}

pub fn part_one(buf_reader: &mut BufReader<File>) {
    let nice_count = buf_reader.by_ref().lines().filter_map(Result::ok).filter(|x| verify_niceness(&x)).count();
...
pub fn verify_niceness2(candidate_string: &str) -> bool {
    let pairs = candidate_string.chars().zip(candidate_string.chars().skip(1));
    let mut nice2a = false;
    let mut nice2b = false;
    ...
    let mut spaced_pairs = candidate_string.chars().zip(candidate_string.chars().skip(2));
    nice2b = spaced_pairs.any(|(a, b)| a == b);
    nice2a && nice2b
}

pub fn part_two(buf_reader: &mut BufReader<File>) {
    println!("Reached part two");
    let nice_count = buf_reader.by_ref().lines().filter_map(Result::ok).filter(|x| verify_niceness2(&x)).count();
    println!{"Part 2 (count of nice strings): {}", nice_count};
}
